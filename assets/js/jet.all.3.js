...
(function () {
  ...
  var _bind = "J",_for = "Jfor",_input = "Jinput",_text = "Jtext",_if = "Jif",_on = "Jon",_run = "Jrun",_attr = "Jattr",_style = "Jstyle",_show = "Jshow",_root = 'Jroot',
    ...
    __ROOT='JET_ROOT',_COMMON_CSS='COMMON_CSS'
  //*********定义全局变量 结束 *******************/
  ...
  window.Jet = function (par, ele, opt) {
    ...
    if(ele===__router_comp){
      Jet.$unnamedJets[__router_comp] = this;
    }
    if (opt.par) {
  ...
  function _initRoot(){
    _JT.id(__ROOT)._JT_exist(function(root){
      root.attr(_jload,Jet.router.conf.base+'/root.html').attr(_jpath,'false');
      _initSingleLoad('$root',root,0,[root],function(){
        root._JT_removeAttr(__comp_id)
      })
    })
  }
  Jet.router = {
  ...
  function _callUnmountLife(calls){
    var _ra;
    if(calls=='__onroute'){
      _ra=Jet.$unnamedJets[__router_comp];
      if(!_ra)
        return;
  ...
  function _removeFromJets(item){
    ...
    if(Jet[name]&&Jet[name].$DOM){
      delete Jet[name];
    }else if(Jet.$unnamedJets[name]){
      delete Jet.$unnamedJets[name];
    }
  }
  ...
  function _initSingleLoad(_this,item,i,list,call){
    ...
    var abs=item._JT_attr(_jpath);
    item._JT_removeAttr(_jload+' '+_jpath);
    var src =(abs!=='false')?Jet.res.getSrc(attr, 'html'):attr;
  ...
  function _dealLoadResult(html, item, _this, attr, i, list, call) {
    ...
    if (item._JT_hasAttr('jonload')) {
      ...
      if (par !== null && Jet[par] !== undefined && _checkIsFunc(Jet[par],onload)) {
        Jet[par][onload].call(Jet[par],opt);
      ...
      //   _this.$par[onload].call(_this.$par,opt);
        ...
        if (par !== null) {
          (new Function(onload)).call(Jet[par],opt);
        } else {
    ...
    //使用递归初始化load 会导致子元素的Jload被父元素加载
  ...
  /*lang*********************************************************************************/
  var _lang = "Jlang", _name = "Jname",_jl_name = '';
  ...
  function _addModule(_this, mods) {
